...
def no_vowels(input_string):
    ...
    인풋으로 받는 스트링에서 모든 모음 (a, e, i, o, u)를 제거시킨 스트링을 반환함
        ...
        Examples:
            ...
            ''W lv Pythn!'
    """
    change_list = ['a','A','e','E','i','I','o','O','u','U']
    no_vowel_string = input_string.replace('a','').replace('e','').replace('i','').replace('o','').replace('u','').replace('A','').replace('E','').replace('I','').replace('O','').replace('U','')
...
ex2 = "We love Pythoaan!"
if __name__ == "__main__":
    result2 = no_vowels(ex2)
    print(result2)